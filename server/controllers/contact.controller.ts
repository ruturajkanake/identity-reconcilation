...
import { getExistingContact, consolidatedContacts, getAllLinkedContacts } from "../helpers/contact.helper";
...
export const createContact = async (req: Request, res: Response) => {
    try {
        ...
        if(!phoneNumber && !email) {
            return BAD_REQUEST(res, 'Please provide either a phone number or email');
        }
        ...
        if(existingContact.length === 0) {
            ...
            const primaryContactId = existingContact[0].linkPrecedence === 'primary' ? existingContact[0].id : existingContact[0].linkedId;
            const linkedContacts = await getAllLinkedContacts(primaryContactId);
            result = consolidatedContacts(linkedContacts);
            if(existingContact[0].phoneNumber !== phoneNumber || existingContact[0].email !== email) {
            ...
            const duplicateContacts = existingContact.filter(contact => contact.phoneNumber === phoneNumber && contact.email === email);
            if(duplicateContacts.length > 0) {
                const primaryContactId = duplicateContacts[0].linkPrecedence === 'primary' ? duplicateContacts[0].id : duplicateContacts[0].linkedId;
                const linkedContacts = await getAllLinkedContacts(primaryContactId);
                result = consolidatedContacts(linkedContacts);
            } else {
                // Multiple existing contacts. Link the new contact to the primary contact
                const primaryContacts = existingContact.filter(contact => contact.linkPrecedence === 'primary')
                const primaryContactId = primaryContacts[0].id;
                const otherPrimaryContacts = primaryContacts.slice(1);
                const otherPrimaryContactIds = otherPrimaryContacts.map(contact => contact.id);
                if(otherPrimaryContacts.length > 0) {
                    ...
                    result = consolidatedContacts([...primaryContacts, ...updatedSecondaryContacts]);
                } else {
